...
- `TUTORIAL_GROUP` must be **3 to 5 alphanumeric characters** (letters or digits only). Letter casing is ignored for validation; the value is **stored in uppercase** (e.g. `t01` and `T01` both become `T01`). Examples: `T01`, `t01`, `CS204` ✓; `ab` (too short), `T01234` (too long), `T-01` ✗
...
* **Input restrictions**: same format as `TUTORIAL_GROUP` in `add` / `edit` (3–5 alphanumeric characters). Values are **stored in uppercase**; you may type any letter casing and filtering still matches.
...
* `TUTORIAL_GROUP` uses the same 3–5 alphanumeric rule as in `add` / `edit` / `find` (e.g. `T01`, `Lab2`). Letter casing is ignored; values match the **uppercase** form stored for each student.
